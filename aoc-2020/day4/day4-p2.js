  ...
  let isValid = matches.every((match, index) => {
    ...
    switch (key) {
      case 'cid':
        ...
        return true
      case 'byr':
        if (!val.match(/19[2-9][0-9]|20[0][0-2]/g)) return false;
        fieldsFulfilled += 1;
        // console.log('GOOD', key, val)
        return true
      case 'iyr':
        if (!val.match(/201[0-9]|2020/g)) return false;
        fieldsFulfilled += 1;
        // console.log('GOOD', key, val)
        return true;
      case 'eyr':
        if (!val.match(/202[0-9]|2030/g)) return false;
        fieldsFulfilled += 1;
        // console.log('GOOD', key, val)
        return true;
      case 'hgt':
        if (val.match(/cm/g) && !val.match(/1[5-8][0-9]|19[0-3]/g)) return false;
        if (val.match(/in/g) && !val.match(/59|6[0-9]|7[0-6]/g)) return false;
        if (!val.match(/cm|in/g)) return false;
        fieldsFulfilled += 1;
        // console.log('GOOD', key, val)
        return true;
      case 'ecl':
        if (!val.match(/amb|blu|brn|gry|grn|hzl|oth/g)) return false;
        fieldsFulfilled += 1;
        // console.log('GOOD', key, val)
        return true;
      case 'hcl':
        if (!val.match(/#[0-9a-fA-F]{6}/g)) return false;
        fieldsFulfilled += 1;
        // console.log('GOOD', key, val)
        return true;
      case 'pid':
        if (!val.match(/[0-9]{9}/g)) return false;
